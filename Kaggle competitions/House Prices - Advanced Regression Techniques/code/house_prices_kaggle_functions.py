...
def transform(df):
    ...
    # BINNING FEATURE 'OverallQual'
    df['OverallQual_bin'] = pd.cut(df['OverallQual'], bins=[0, 5, 8, 11], labels=['LowQual', 'MediumQual', 'HighQual'])
    
    
    # DUMMIES FOR BINNING FEATURE
    df_overall_qual_dummies = pd.get_dummies(df['OverallQual_bin'], prefix='OverallQual', drop_first=True)
    
    
    # CONCATE
    df_transformed = pd.concat([df[columnas_quedar], df_overall_qual_dummies], axis=1)
